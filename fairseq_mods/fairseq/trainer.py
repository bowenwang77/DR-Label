...
class Trainer(object):
        ...
        if bexists:
            ...
            try:
                self.model.load_state_dict(
                    state["model"], strict=not not_strict_parameter, model_cfg=self.cfg.model
                )
                # save memory for later steps
                del state["model"]
                if utils.has_parameters(self.get_criterion()):
                    self.get_criterion().load_state_dict(
                        state["criterion"], strict=not not_strict_parameter
                    )
                    del state["criterion"]
    ...
    def test_step(self, sample, raise_oom=False):
        ...
        with torch.no_grad():
            ...
            try:
                sid, energy, sample_size, _ = self.task.test_step(
                    sample, self.model, self.criterion, **extra_kwargs
                )
            except RuntimeError as e:
                if "out of memory" in str(e):
                    ...
                    if not raise_oom:
                        ...
                        if self.cuda:
                            torch.cuda.empty_cache()
                        return self.test_step(sample, raise_oom=True)
                raise e
